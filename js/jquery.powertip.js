...
(function(root, factory) {
	// support loading the plugin via common patterns. Suporte carregando o plugin via padrões comuns
	if (typeof define === 'function' && define.amd) {
		// load the plugin as an amd module. Carregar o plugin como um módulo amd
		define([ 'jquery' ], factory);
	} else if (typeof module === 'object' && module.exports) {
		// load the plugin as a commonjs module. Carregar o plugin como um módulo commonjs
		module.exports = factory(require('jquery'));
	} else {
		// load the plugin as a global. Carregar o plug-in como um
		factory(root.jQuery);
	}
}(this, function($) {

	// useful private variables. Variáveis privadas úteis
	...
	/**
	 * Session data
	 * Private properties global to all powerTip instances. Propriedades particulares globais para todas as instâncias do powerTip
	 */
	var session = {
		elements: null,
		tooltips: null,
	...
	/**
	 * Display hover tooltips on the matched elements. Exibe dicas de ferramenta sobre os elementos correspondentes.
	 * @param {(Object|string)=} opts The options object to use for the plugin, or
	 *     the name of a method to invoke on the first matched element.
	 * @param {*=} [arg] Argument for an invoked method (optional). Argumento para um método invocado (opcional).
	 * @return {jQuery} jQuery object for the matched selectors. JQuery para os seletores correspondentes.
	 */
	$.fn.powerTip = function(opts, arg) {
		...
		// don't do any work if there were no matched elements. Não faça nenhum trabalho se não houver elementos correspondentes
		...
		// hook mouse and viewport dimension tracking. Rastreio de dimensão do mouse e do visor do gancho
		initTracking();
